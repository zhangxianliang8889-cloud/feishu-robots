...
def main():
    print("=" * 80)
    print("🤖 统一测试框架 V2 - 策划二部群内容测试")
    print("   确保使用正确的机器人身份发送消息")
    print("=" * 80)
    ...
    print(f"\n📋 Token信息：")
    print(f"   群会议纪要Token: {meeting_token[:40]}...")
    print(f"   群消息统计Token: {stats_token[:40]}...")
...
💡 提示：
   请检查测试群中的消息发送者身份：
   - 前3条消息应该显示为"群会议纪要"机器人发送
   - 后3条消息应该显示为"群消息统计"机器人发送
   
   如果都显示同一个机器人，可能原因：
   1. 群消息统计机器人没有加入测试群
   2. 飞书后台配置问题
   3. 飞书API缓存问题（等待几分钟后重试）
""")
